 ...
 * @author Zhang Xu
 */
public class NaviRoundRobinServiceSelector implements NaviServiceSelector {

	private final static Logger log = LoggerFactory.getLogger(NaviRoundRobinServiceSelector.class);
	...
	public NaviRoundRobinServiceSelector(List<NaviServiceInvoker> serviceList, int retryTimes, Method method, Object[] args) {
	...
	public Object execute(NaviFailStrategy failStrategy) throws RpcInvocationException {
		...
		int start = counter.incrementAndGet() % random.size();
		for (int retry = 0; retry < retryTime && retry < random.size();) {
			NaviServiceInvoker service = random.get(start);
			start++;
			start %= random.size();
			try {
